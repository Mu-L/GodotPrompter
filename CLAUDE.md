...
- **Version lockstep:** `release.yml` verifies **five** files — `package.json`, the root `plugin.json` (Antigravity), `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, and `.cursor-plugin/plugin.json`. `scripts/bump-version.mjs <version>` updates all five, plus both version fields in the committed `package-lock.json`, and syncs the skill-count description.
...
- **`.github/workflows/plugin-scan.yml`** runs the HOL plugin-scanner (the awesome-ai-plugins listing check) on every push to `master` and every PR, and fails below 80/100. It flags the Codex full-access sandbox mode name and a never-ask approval policy written literally in any `.md`/`.json`/`.toml`/`.yml` file, comments and this file included (patterns: `RISKY_APPROVAL_PATTERNS` in hol-guard's `checks/security.py`), so describe them rather than quoting them. Every `uses:` must be pinned to a full commit SHA — Dependabot keeps the pins current.
